    ...
    <>
      <header className="navbar">
        ...
        <nav className="navbar-links">
          {/* 🌟 ป้ายชื่อสำหรับ Desktop */}
          <NavLink id="tour-dashboard-desktop" to="/" end className={({ isActive }) => (isActive ? 'active' : '')} >
            {t('dashboard')}
          </NavLink>
          <NavLink id="tour-categories-desktop" to="/categories" className={({ isActive }) => (isActive ? 'active' : '')}>
            {t('categories')}
          </NavLink>
        </nav>

        {/* 🌟 ป้ายชื่อสำหรับ Desktop */}
        <div className="navbar-pro-container" id="tour-upgrade-desktop">
          {isPro ? (
            <div 
        ...
        <div className="navbar-user">
          <button className="btn btn-ghost btn-small" onClick={toggleLang} style={{ fontWeight: 'bold', minWidth: '50px', marginRight: '8px' }}>
            {lang === 'en' ? 'English' : 'ไทย'}
          </button>
          <span className="navbar-email">{user.email}</span>
          
          {/* 🌟 ป้ายชื่อสำหรับ Desktop */}
          <button id="tour-settings-desktop" className="btn btn-ghost btn-small" onClick={() => navigate('/settings')} style={{ marginRight: '8px' }}>
            ⚙️ {t('settings')}
          </button>
      ...
      <nav className="bottom-nav">
        {/* 🌟 ป้ายชื่อสำหรับ Mobile */}
        <NavLink id="tour-dashboard-mobile" to="/" end className={({ isActive }) => (isActive ? 'active' : '')}>
        ...
        {/* 🌟 ป้ายชื่อสำหรับ Mobile */}
        <NavLink id="tour-categories-mobile" to="/categories" className={({ isActive }) => (isActive ? 'active' : '')}>
          <svg className="icon" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* 🌟 ป้ายชื่อสำหรับ Mobile */}
        <NavLink id="tour-settings-mobile" to="/settings" className={({ isActive }) => (isActive ? 'active' : '')}>
          <svg className="icon" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
